...
import { TokenAddressEthereum, TokenAddressArbitrum, TokenAddressBSC, TokenAddressPolygonPOS } from './server/config/config-token-addresses';
...
export const myConfigOverrides = () => {
  ...
  configDefaults.poi.nodeURL = ['https://ppoi-agg.horsewithsixlegs.xyz/'];
  // configDefaults.poi.nodeURL = ['https://ppoi-agg.horsewithsixlegs.xyz']; # later test with no / if polling shows
  ...
  const ethereumProviders = createFallbackProviderConfig( 
    NetworkChainID.Ethereum, 
    [ 
      'https://eth-mainnet.g.alchemy.com/v2/paYzNZ8yRPtpEtk5UgqflVgkMqLt1eK8'
    ], 
  ); 
  setFallbackProviderConfigForNetworkEVM(NetworkChainID.Ethereum, ethereumProviders);

  // BNB Smart Chain RPC providers
  const bscProviders = createFallbackProviderConfig( 
    NetworkChainID.BNBChain, 
    [ 
      // 'https://bsc-dataseed.bnbchain.org'
      // 'https://bsc.publicnode.com'
      'https://bsc-mainnet.core.chainstack.com/b86601586866a460b96c0eab31823385' // developer plan node 
    ], 
  ); 
  setFallbackProviderConfigForNetworkEVM(NetworkChainID.BNBChain, bscProviders);


  setTokensForNetworkEVM(NetworkChainID.Ethereum, { 
    [TokenAddressEthereum.WETH]: { symbol: 'WETH' },
    [TokenAddressEthereum.USDT]: { symbol: 'USDT' },
    [TokenAddressEthereum.USDC]: { symbol: 'USDC' },
  }); 

  setTokensForNetworkEVM(NetworkChainID.BNBChain, { 
    [TokenAddressBSC.WBNB]: { symbol: 'WBNB' },
    [TokenAddressBSC.USDT]: { symbol: 'USDT' },
    [TokenAddressBSC.USDC]: { symbol: 'USDC' },
  });
  ...
  setTopUpUnshieldAmountForNetwork(NetworkChainID.BNBChain, 0.05);
  setTopUpBeginThresholdForNetwork(NetworkChainID.BNBChain, 0.01);

  setNativeTokenAccumulationForNetwork(NetworkChainID.Ethereum, true);
  setMinAvailabilityBalanceForNetwork(NetworkChainID.Ethereum, 0.005);
  setTopUpUnshieldAmountForNetwork(NetworkChainID.Ethereum, 0.01);
  setTopUpBeginThresholdForNetwork(NetworkChainID.Ethereum, 0.005);
};
